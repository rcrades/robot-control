...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    switch (method) {
      case 'GET':
        if (query.id) {
          const robot = await getRobotById(query.id as string);
          if (robot) {
        ...
        break;
      case 'POST':
        const newRobot = await addRobot(body);
        res.status(201).json(newRobot);
        break;
      case 'PUT':
        if (query.id) {
          const updatedRobot = await updateRobot(query.id as string, body);
          if (updatedRobot) {
      ...
      case 'DELETE':
        if (query.id) {
          const deleted = await deleteRobot(query.id as string);
          if (deleted) {
            res.status(204).end();
          } else {
            res.status(404).json({ message: 'Robot not found' });
